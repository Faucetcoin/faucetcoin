...
uint256 hashGenesisBlock("0xe7029a5686189dad6bebc976d962f295b567659360af3a6c3786326c5a1f6899");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // FaucetCoin: starting difficulty is 1 / 2^12
...
int64 CTransaction::nMinTxFee = 3000000;
/** Fees smaller than this (in satoshi) are considered zero fee (for relaying) */
int64 CTransaction::nMinRelayTxFee = 3000000;
...
const string strMessageMagic = "FaucetCoin Signed Message:\n";
...
{
    // FaucetCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	if(nHeight < 10054) // stops generation of more coins
	{
		nSubsidy = 1000 * COIN;
	}
	if(nHeight < 51)
...
static const int64 nTargetTimespan = 600; // FaucetCoin: 10 minutes
static const int64 nTargetSpacing = 60; // FaucetCoin: 60 seconds
static const int64 nInterval = nTargetTimespan / nTargetSpacing;
static const int64 nTargetTimespanRe = 1200; // FaucetCoin: 20 Minutes
static const int64 nTargetSpacingRe = 60; // FaucetCoin: 60 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // FaucetCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // FaucetCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // FaucetCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd8afa5938b0187c97e357dacb3cc32fd430fbaa6f8bfc2576c5dcbc2f18314b1");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times: Protesters in Ukraine’s East Call On Putin to Send Troops";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1396910426;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 3265926;

        if (fTestNet)
        {
            block.nTime    = 1396922937;
            block.nNonce   = 259484;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x5f4c3fa749eace7616244afda0b5f6d0fb5a450e30d2402188f0565ddc4200ae"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // FaucetCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FaucetCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FaucetCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FaucetCoinMiner(CWallet *pwallet)
{
    printf("FaucetCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("faucetcoin-miner");
    ...
    try { loop {
        ...
        printf("Running FaucetCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FaucetCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FaucetCoinMiner, pwallet));
}
